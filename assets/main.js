    ...
    loadGiffy() {
        $(function() {
            ...
            function loadEmote(property) {
                android.append('<img onload="this.style.opacity=' + 1 + ';" ' + property + ' >').css('opacity', '1');
                androidSpinner.css('position', 'absolute');
                android.find('img').on('load', function () {
                    androidSpinner.addClass('fade-background-svg');
                });
            }

            var id = getRandomInt(1, gifCount);
            if (!mobileCheck()) {
                loadEmote('src="' + baseurl + 'droid-' + id + '.gif"');
            } else {
                loadEmote('src="' + baseurl + 'droid-head.png" class="mobile-droid"');
            }
